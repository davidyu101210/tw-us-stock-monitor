...
from src.providers.us_institutional import institutional_reference, concentration_score, format_holders_chinese

st.set_page_config(page_title="台美股看盤 iPhone v1.2", layout="wide")
...
st.title("台股＋美股自動看盤 v1.2 iPhone PWA 版")
...
if market_mode == "台股":
    ...
    def load_us_inst(ticker):
        return institutional_reference(ticker)
    ...
    info2.metric(
        "最新 13F 申報日期",
        st.session_state.last_13f_report_date or "無資料"
    )
...
st.divider()
st.caption("v1.2：iPhone PWA；每日自動檢查 13F，顯示最後檢查時間與最新申報日期。")
